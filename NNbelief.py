...
def beta_jk(w):  # appendix B.3
    ...
    """
    beta_mean = np.mean(w, axis=1, keepdims=True)  # shape (J, 1)
...
def alpha_jk(beta, beta_0, phix):  # Equ.38
    ...
    """
    J, K = beta.shape
    mu = np.mean(phix, axis=0, keepdims=True)  # shape (1, J)
    first_term = (beta_0.reshape(1, K) + np.dot(mu, beta)) / J  # shape (1, K)
...
def weight_pos(beta, alpha, x):  # Equ.27
    ...
    pos_weight = relu(weight).sum(axis=1)  # shape (N, K)
    return pos_weight
...
def weight_neg(beta, alpha, x):
    ...
    """
    weight = belief_weight(beta, alpha, x)  # shape (N, J, K)
    neg_weight = relu(-weight).sum(axis=1)  # shape (N, K)
    return neg_weight
...
def eta_pos(beta, alpha, x):
    ...
    K = w_pos.shape[-1]
    eta_pos = 1 / (np.exp(w_pos).sum(axis=1) - K + 1)  # shape (N,)
    return eta_pos
...
def eta_neg(beta, alpha, x):
    ...
    eta_neg = 1 / (1 - np.prod(1 - np.exp(-w_neg), axis=1))  # shape (N,)
    return eta_neg
...
def conflict(beta, alpha, x):
    ...
    kappa = np.sum(eta_pos_temp.reshape(-1, 1) * (np.exp(w_pos) - 1) *
                   (1 - eta_neg_temp.reshape(-1, 1) * np.exp(-w_neg)),
                   axis=1)  # shape (N,)
    return kappa
...
def ignorance(beta, alpha, x):
    ...
    ig_temp = eta_temp * eta_pos_temp * eta_neg_temp * np.exp(
        -np.sum(w_neg, axis=1))
    return ig_temp
...
def m_theta_k(beta, alpha, x):  # return the masses for all k
    ...
    first_term = eta_mul.reshape(-1, 1) * np.exp(-w_neg)  # shape (N, K)
    prod_term = np.prod(1 - np.exp(-w_neg), axis=1, keepdims=True) / (
        1 - np.exp(-w_neg))  # shape (N, 1) / shape (N, K) = shape (N, K)
    second_term = np.exp(w_pos) - 1 + prod_term  # shape (N, K)
...
def my_floor(a, precision=0):
    """[summary]
       Theoretically, 1-sum(m(\theta_k))should be greater than 0. However, due to the numerical precision error, there would appear the opposite situation. Thus, we define this floor function.
...
def get_k_ig(beta, alpha, x):
    ...
    # weight = weight / 0.4
    w_pos = relu(weight).sum(axis=1)  # shape (N, K)
    w_neg = relu(-weight).sum(axis=1)  # shape (N, K)
    import pdb; pdb.set_trace()
    K = w_pos.shape[-1]

    eta_pos_temp = 1 / (np.exp(w_pos).sum(axis=1) - K + 1)  # shape (N,)
    eta_neg_temp = 1 / (1 - np.prod(1 - np.exp(-w_neg), axis=1))  # shape (N,)
    kappa = np.sum(eta_pos_temp.reshape(-1, 1) * (np.exp(w_pos) - 1) *
                   (1 - eta_neg_temp.reshape(-1, 1) * np.exp(-w_neg)),
                   axis=1)  # shape (N,)
    ...
    non_specificity_simple = 1 - my_floor(m_theta_k, precision=15).sum(axis=1)
...
def get_k_ig_ns(beta, alpha, x):
    ...
    eta_mul = eta_temp * eta_pos_temp * eta_neg_temp  # shape (N,)
    first_term = eta_mul.reshape(-1, 1) * np.exp(-w_neg)  # shape (N, K)
    prod_term = np.prod(1 - np.exp(-w_neg), axis=1, keepdims=True) / np.maximum((1 - np.exp(-w_neg)), 1e-25)  # shape (N, 1) / shape (N, K) = shape (N, K)
    # prod_term = np.prod(1 - np.exp(-w_neg), axis=1, keepdims=True) / (
    #     1 - np.exp(-w_neg))  # shape (N, 1) / shape (N, K) = shape (N, K)
    second_term = np.exp(w_pos) - 1 + prod_term  # shape (N, K)
    m_theta_k = first_term * second_term  # shape (N, K)
    # non_specificity_simple = 1 - my_floor(m_theta_k, precision=15).sum(axis=1)
    non_specificity_simple = 1 - m_theta_k.sum(axis=1)
...
def m_theta_A(beta, alpha, x, A):
    ...
    """
    K = alpha.shape[1]
    u_set = np.arange(K)  # Universal set
    ...
    w = np.prod(np.exp(-w_neg[:, A]), axis=1)
    _w = np.prod(1 - np.exp(-w_neg[:, A_comp]), axis=1)
...
def plausibility(beta, alpha, x, u_set, A):
    ...
    pl = 1 - np.round(bel(beta, alpha, x, com_A), 14)
    return pl
...
def plausibility_transform(beta, alpha, x, u_set):
    ...
    pl = np.array(pl)
    pl_sum = pl.sum(axis=0, keepdims=True)
    pm = pl / pl_sum
    return pm
...
def H_conflict(beta, alpha, x, u_set):
    ...
    N = x.shape[0]
    sum_pl = np.zeros(N)  # normalization constant for plausibility transform
    p_m = []
    for theta_k in u_set:
...
def H_non_specificity(beta, alpha, x, u_set):
    ...
    for subset in powset:
        ...
        if set_size > 1:
            h_temp.append(
                m_theta_A(beta, alpha, x, subset) * np.log2(set_size))
